package com.luoshengwei.common.core.mybatis.interceptor;
...
import com.luoshengwei.common.core.mybatis.dialect.Dialect;

/**
 * @ClassName:ExecutorInterceptor
 * @Description:(Mybatis查询拦截器)
 * @author:luoshengwei
 * @date:2018-4-6 下午4:41:23
	...
	private void processIntercept(final Object[] queryArgs) {
		...
		if (dialect.supportsLimit() && (offset != RowBounds.NO_ROW_OFFSET || limit != RowBounds.NO_ROW_LIMIT)) {
			...
			String sql = boundSql.getSql().replaceAll("\\s{2,}", " ").replace(" FROM", " from")
					.replace("ORDER BY", "order by").replace("GROUP BY", "group by").trim();
